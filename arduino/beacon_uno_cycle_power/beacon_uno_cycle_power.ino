...
void radioon(int pwr){
  ...
  Serial.print("Set power to "); // Using 5 as high SF + high power isn't being received 
  Serial.println(pwr);
...
void beacon(int pwr){
  ...
  snprintf(radiopacket,
           ...
           "BEACON %s %s VCC=%d.%03d count=%d uptime=%ld pwr=%d",
           CALLSIGN,
           COMMENTS,
           (int) vcc,
           (int) (vcc*1000)%1000,
